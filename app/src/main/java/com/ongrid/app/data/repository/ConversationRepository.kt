...
private fun MessageEntity.toChatMessage(): ChatMessage {
    val isTyping = role == "TYPING"
    val isToolError = role == "TOOL_ERROR"
    return ChatMessage(
        id = id,
        role = when {
            isTyping -> MessageRole.ASSISTANT
            isToolError -> MessageRole.TOOL   // renders as a tool bubble, just red
            else -> MessageRole.valueOf(role)
        },
        content = content,
